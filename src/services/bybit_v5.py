    ...
    async def _safe_api_call(self, func, *args, **kwargs) -> Any:
        ...
        for attempt in range(3):
            try:
                ...
                logger.warning(f"[X-RAY] ⚠️ API Call to {func_name} timed out after 10s. Retrying ({attempt+1}/3)...")
                await asyncio.sleep(1.0)
                if attempt == 2:
                    raise Exception(f"API Call Timeout for {func_name} after 3 attempts.")
                continue
                ...
                if "110126" in error_str:
                    ...
                    raise ValueError(f"[X-RAY] 🚫 110126 INNOVATION ZONE: {symbol_banned} temporarily banned for 15m.")
