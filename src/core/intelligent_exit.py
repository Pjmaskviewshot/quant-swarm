...
class PositionExitState:
    ...
    execution_state: str = "OBSERVE"  # Defaults to OBSERVE to prevent SYNC deadlocks[cite: 1]
    exec_order_id: str = ""
...
@dataclass
class ExitDecision:
    action: str  # "HOLD" | "EXIT" | "SCALE_OUT" | "EMERGENCY"[cite: 1]
    target_q: float
    urgency: str
    limit_price: float
...
class IntelligentExitEngine:
    ...
    def evaluate(ctx: Dict[str, Any], state: PositionExitState) -> ExitDecision:
        ...
        # 1. Base Volume Self-Healing[cite: 1]
        if state.actual_qty <= 0.0 and state.base_qty > 0.0:
            state.actual_qty = state.base_qty

        # Auto-recovering SYNC lock with 2.0-second safety timeout[cite: 1]
        ...
        # Physical executable top-of-book pricing (BBO)[cite: 1]
        ob = ctx.get("last_ob", {})
        ...
        # Systemic Portfolio Drawdown Hard-Stop[cite: 1]
        ...
        # Maximum Holding Horizon (240 Minutes Hard Cap)[cite: 1]
        ...
        # Initial risk distance anchored to entry to eliminate denominator jitter[cite: 1]
        ...
        # 2. Path Telemetry (MFE and MAE Tracking)[cite: 1]
        ...
        # =========================================================================
        # CONTINUOUS OPTIMAL STOPPING SENSORS (Active Strictly in Profit)[cite: 1]
        # =========================================================================
        ...
        if stat_engine:
            ...
            # SENSOR TRIGGER 1: Alpha Drift Inversion (>= 0.50R Profit)[cite: 1]
            if current_r >= 0.50:
            ...
            # SENSOR TRIGGER 2: Kinematic Cascade Exhaustion (>= 0.70R Profit)[cite: 1]
            if current_r >= 0.70:
            ...
            # SENSOR TRIGGER 3: Bayesian Regime Termination (>= 0.80R Profit)[cite: 1]
        ...
        # SENSOR TRIGGER 4: Dynamic Profit Retracement Guard (>= 0.80R Peak)[cite: 1]
        ...
        # Replaces noisy sub-minute Hurst exponent with robust relative volatility ratio
        ...
        # =========================================================================
        # TIERED FRACTIONAL SCALE-OUT (50% lot at 1.4R)[cite: 1]
        # =========================================================================
        ...
        # =========================================================================
        # PHYSICAL BREACH EXECUTION (Evaluated against Top-of-Book BBO)[cite: 1]
        # =========================================================================
        # Trailing Stop Breaches[cite: 1]
        ...
        # Take-Profit Limit/Market Breaches[cite: 1]
...
class ExecutionGovernorFSM:
    ...
    async def manage_execution(cls, decision: ExitDecision, state: PositionExitState, ctx: Dict[str, Any], executor: Any) -> bool:
        ...
        # Market IOC execution for emergency, full exits, and partial scale-outs[cite: 1]
        if decision.urgency in ["MARKET", "EMERGENCY", "AGGRESSIVE", "FLASH_IOC"]:
            ...
            # Verify exchange acceptance before altering local state[cite: 1]
        ...
        # Passive Limit PostOnly execution[cite: 1]
